...
public class GvrEditorEmulator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (camera && camera.enabled && camera.stereoTargetEye != StereoTargetEyeMask.None)
        {
            if (camera.nearClipPlane > 0.1
                && GvrSettings.ViewerPlatform == GvrSettings.ViewerPlatformType.Daydream)
            {
                Debug.LogWarningFormat(
                    ...
                    camera.name, camera.nearClipPlane);
            }
        }
    ...
    private void ApplyHeadOrientationToVRCameras()
    {
        UpdateAllCameras();
            ...
            if (camera && camera.enabled && camera.stereoTargetEye != StereoTargetEyeMask.None)
            {
            camera.transform.localPosition = HeadPosition * camera.transform.lossyScale.y;
            camera.transform.localRotation = HeadRotation;
        }
    }
